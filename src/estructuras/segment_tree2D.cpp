int n,m;
int a[MAXN][MAXN],st[4*MAXN][4*MAXN];
void build(){
	forr(i,0,n)forr(j,0,m)st[i+n][j+m]=a[i][j];
	forr(i,0,n)for(int j=m-1;j;--j)
		st[i+n][j]=op(st[i+n][j<<1],st[i+n][j<<1|1]);
	for(int i=n-1;i;--i)forr(j,0,2*m)
...
int query(int x0, int x1, int y0, int y1){
	...
	for(int i0=x0+n,i1=x1+n;i0<i1;i0>>=1,i1>>=1){
		...
		if(i0&1)t[q++]=i0++;
		if(i1&1)t[q++]=--i1;
		forr(k,0,q)for(int j0=y0+m,j1=y1+m;j0<j1;j0>>=1,j1>>=1){
